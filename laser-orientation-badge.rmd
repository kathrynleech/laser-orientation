...
-   There is some missing data which is indicated by NA. 
...
-   Rows are typically the unit of observation, such as a person, school, etc. 

What columns typically (or you think may) represent:

-   Columns are typically the attributes about the units of observation, and most often serve as independent and dependent variables in analyses. 
...
```{r}
d %>%
  select(student_id, total_points_possible, total_points_earned, int)
```
...
-   I changed the cut off from >70 to >98. Now the data is filtered to only include rows where final grades are greater than 98. 
